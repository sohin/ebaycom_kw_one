...
while (true)
{
    ...
    foreach($results as $data)
    {
        ...
        $timeLeftn = $cd->find("span[@class='vi-is1-tml']", 0);
        $timeLeft  = '';
        if (isset($timeLeftn)) 
        $timeLeft = trim(html_entity_decode($timeLeftn->plaintext, ENT_QUOTES, 'UTF-8'));  
...
/////////////////////
        ...
        $numberOfBid = $cd->find("span[@class='vi-is1-tese']", 0)->plaintext;
        $numberOfBid = trim(html_entity_decode($numberOfBid, ENT_QUOTES, 'UTF-8'));

        $shipping = $cd->find("span[@class='vi-is1-tese']", 0)->plaintext;
        $shipping = trim(html_entity_decode($shipping, ENT_QUOTES, 'UTF-8'));
        ...
        $payments = $cd->find("span[@class='vi-is1-tese']", 0)->plaintext;
        $payments = trim(html_entity_decode($payments, ENT_QUOTES, 'UTF-8'));

        $sellerID = $cd->find("span[@class='mbg-nw']", 0)->plaintext;
        $sellerID = trim(html_entity_decode($sellerID, ENT_QUOTES, 'UTF-8'));

        $description = $cd->find("div[@class='tb-cntOn']", 0)->plaintext;
...
/////////////////////
        ...
        $images = $cd->find("img[@class='img img400']", 0)->href; 
        $imageurl = scraperWiki::scrape($images);
